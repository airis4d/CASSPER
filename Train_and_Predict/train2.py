...
model_checkpoint_name=args.dataset+"checkpoints/latest_model_"+ args.frontend + "_" + args.model + "_" + args.dataset + ".ckpt"
old_epoch=0
if args.continue_training:
    print('Loaded latest model checkpoint')
    saver.restore(sess, model_checkpoint_name)
    old_epoch=args.epoch_start_i
...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    if epoch % args.validation_step==0:
        ...
        for ind in val_indices:
            ...
            tfp="%s%s/%04d/%s_pred.png"%(args.dataset,"checkpoints",epoch,file_name)
            tfg="%s%s/%04d/%s_gt.png"%(args.dataset,"checkpoints",epoch, file_name)
    ...
    p=(index,value,args.model)
    import pickle
    print("restoring model")
    model_checkpoint_name=args.dataset+"checkpoints/latest_model_"+ args.frontend + "_" + args.model + "_" + args.dataset + ".ckpt"
    saver.restore(sess, model_checkpoint_name)
    try:
        j,m,k=pickle.load(open("%s/TSaved/Frn_%s_Mod_%s_BestTR.p"%(args.dataset,args.frontend,args.model),'rb'))
        if((value > m) and k==args.model):
            print("The highest accuracy was observed for epoch %s and is %s"%(index,value))
            pickle.dump(p, open("%s/TSaved/Frn_%s_Mod_%s_BestTR.p"%(args.dataset,args.frontend,args.model),'wb'))
            #if(index==epoch):
            print("Saving model")
            saver.save(sess,"%s/TSaved/BestFr_%s_model_%s.ckpt"%(args.dataset,args.frontend,args.model))
    ...
    p=(index,valueF,args.model)
    import pickle
    try:
        j,m,k=pickle.load(open("%s/TSaved/Frn_%s_Mod_%s_BestTR_F1.p"%(args.dataset,args.frontend,args.model),'rb'))
        if((valueF > m) and k==args.model):
            print("The highest F1 score was observed for epoch %s and is %s"%(index,valueF))
            pickle.dump(p, open("%s/TSaved/Frn_%s_Mod_%s_BestTR_F1.p"%(args.dataset,args.frontend,args.model),'wb'))
            #if(index==epoch):
            print("Saving model")
            saver.save(sess,"%s/TSaved/BestFr_%s_model_%s_F1.ckpt"%(args.dataset,args.frontend,args.model))
